...
function App() {
  ...
  const handleSubmit = async () => {
    try {
        const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/process`, {
            filename: file.name,
        ...
        if (response.data.filename) {
            setDownloadLink(`${process.env.REACT_APP_BACKEND_URL}/download/${response.data.filename}`);
            setIsEditing(false);
        } else {
...
const handleDownload = async () => {
    try {
        ...
        // Handling the download response as before
    } catch (error) {
        ...
        {!downloadLink ? (
          <div className="space-y-6 max-w-2xl mx-auto mt-8">
              ...
              <div className="flex justify-center space-x-2">
                {['General', 'Exam', 'Data Structures'].map((type) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-full ${promptType === type ? 'bg-primary text-white' : 'bg-opacity-20 bg-other text-white hover:bg-opacity-40'
                      }`}
                  >
                    {type}
                  </button>
            ...
            <div>
              ...
              <div
                {...getRootProps()}
                className={`p-20 border-2 border-dashed rounded-lg text-center cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-primary bg-opacity-10' : 'border-gray-300 hover:border-primary-light'
                  }`}
              >
            ...
            <button
              ...
              className={`w-full bg-primary hover:bg-primary-light text-white font-bold py-3 px-4 rounded-full transition duration-300 ease-in-out flex items-center justify-center ${(!file || isUploading) ? 'opacity-50 cursor-not-allowed' : ''
                }`}
            >
              {isUploading ? (
                'Uploading...'
